...
public class SettingsController : MonoBehaviour
{
    ...
    void HandleLivesForward(){
        if(livesIndex < livesObjects.Count -1) livesIndex++;
        else livesIndex = 0;
    ...
    void HandleLivesBack(){
        if(livesIndex > 0) livesIndex--;
        else livesIndex = livesObjects.Count -1;
    ...
    void HandleTimeForward(){
        if(timeIndex < timerObjects.Count -1) timeIndex++;
        else timeIndex = 0;
    ...
    void HandleTimeBack(){
        ...
        else timeIndex = timerObjects.Count -1;
    ...
    void DisplayObjectAtIndex(List<GameObject> list, int index){
        for(int i=0; i<list.Count; i++){
            if(i == index) list[i].SetActive(true);
